...
def get_accuracy_error(outputs, targets):
    """
    Raw count of misclassified branches (predicted label != true label),
    matching 1-accuracy. Returns a sum (not a mean) so callers can
    accumulate across batches and divide by the total row count.
    """
    y_k = outputs.squeeze(-1) if outputs.dim() > 1 else outputs
    preds = (y_k > 0.5).float()
